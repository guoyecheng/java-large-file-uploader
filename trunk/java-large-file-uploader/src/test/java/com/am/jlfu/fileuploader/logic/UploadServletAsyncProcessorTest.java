...
public class UploadServletAsyncProcessorTest {
	...
	// @Test
	// public void testStreamDisconnection() throws IOException, InterruptedException,
	// InvalidCrcException {
	// processStreamDisconnection(true, false);
	// }
	//
	// @Test
	// public void testStreamDisconnectionInSecondSlice() throws IOException, InterruptedException,
	// InvalidCrcException {
	// processStreamDisconnection(false, false);
	// }
	//
	// @Test(expected = InvalidCrcException.class)
	// public void testStreamDisconnectionWithBadCrcSignature() throws IOException,
	// InterruptedException, InvalidCrcException {
	// processStreamDisconnection(true, true);
	// }
	//
	// @Test(expected = InvalidCrcException.class)
	// public void testStreamDisconnectionWithBadCrcSignatureSecondSlice() throws IOException,
	// InterruptedException, InvalidCrcException {
	...
	// if (i++ == UploadProcessor.sliceSizeInBytes /
	// UploadServletAsyncProcessor.SIZE_OF_THE_BUFFER_IN_BYTES / 2) {
	...
	// public void processStreamDisconnection(final boolean disconnectInFirstPart, boolean failure)
	...
	// ByteArrayInputStream byteStream1 = (disconnectInFirstPart ? new
	// ByteArrayInputStreamThatFails(fileSlice1Content.clone()) : new
	// ByteArrayInputStream(fileSlice1Content.clone()));
	// ByteArrayInputStream byteStream2 = (!disconnectInFirstPart ? new
	// ByteArrayInputStreamThatFails(fileSlice2Content.clone()) : new
	// ByteArrayInputStream(fileSlice2Content.clone()));
	//
	// // prepare an upload
	...
	//
	// TestFileSplitResult byteArrayFromFile = UploadProcessorTest.getByteArrayFromFile(file, 0,
	// UploadProcessor.sliceSizeInBytes);
	// uploadServletAsyncProcessor.process(fileId, byteArrayFromFile.crc, byteArrayFromFile.stream,
	// new Listener(semaphore));
	// semaphore.tryAcquire(10, TimeUnit.MINUTES);
	// completionInBytes =
	// uploadProcessor.getConfig().getPendingFiles().get(fileId).getFileCompletionInBytes();
	//
	//
	// } else {
	//
	...
	// }
	// emulatedFrontEndFile = UploadProcessorTest.getByteArrayFromFile(file, crcedBytes,
	// completionInBytes - crcedBytes);
	...
	// if (disconnectInFirstPart) {
	//
	// TestFileSplitResult byteArrayFromFile = UploadProcessorTest.getByteArrayFromFile(file,
	// completionInBytes, size);
	// uploadServletAsyncProcessor.process(fileId, byteArrayFromFile.crc, byteArrayFromFile.stream,
	// new Listener(semaphore));
	// semaphore.tryAcquire(10, TimeUnit.MINUTES);
	//
	...
	// completionInBytes -= 10;
	// }
	// emulatedFrontEndFile = UploadProcessorTest.getByteArrayFromFile(fileSlice2, 0,
	// completionInBytes - crcedBytes);
	// fileUploadConfigurationOfFrontEndFile = new FileUploadConfiguration();
	// fileUploadConfigurationOfFrontEndFile.setInputStream(emulatedFrontEndFile.stream);
	// fileUploadConfigurationOfFrontEndFile.setFileId(fileId);
	...
	// String valueSource = crcHelper.getBufferedCrc(new
	// ByteArrayInputStream(fileContent)).getCrcAsString();
	//
	...
	// String valueCopied = crcHelper.getBufferedCrc(new FileInputStream(new
	// File(absoluteFullPathOfUploadedFile))).getCrcAsString();
	//
	// // assert the same
