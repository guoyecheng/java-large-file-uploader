...
public class StaticStateManagerServiceTest {
	...
	public void progressCalculationTest() {
		...
		Assert.assertThat(Double.valueOf(30), is(staticStateManagerService.calculateProgress(30l, 100l)));
		Long bigValue = 1000000000000000000l;
		// check that we dont return 100% if values are not exactly equals
		Assert.assertThat(Double.valueOf(100), is(not(staticStateManagerService.calculateProgress(bigValue - 1, bigValue))));
		// check that we return 100% if values are equals
		Assert.assertThat(Double.valueOf(100), is(staticStateManagerService.calculateProgress(bigValue, bigValue)));
		// check that we return 0% when 0/x
		Assert.assertThat(Double.valueOf(0), is(staticStateManagerService.calculateProgress(0l, 240l)));
	}
}
