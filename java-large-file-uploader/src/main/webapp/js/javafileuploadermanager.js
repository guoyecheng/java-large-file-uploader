...
var globalServletMapping = "javaFileUploaderManagerServlet";

var pendingFiles = new Object();

/*
 * callback: Callback function with the following parameters:
 * 
 * -First argument: A map of the files previously uploaded. The key of this map
 * is the fileIdentifier. The value is a pendingFile: This object contains:
 * -fileComplete (boolean) : specifies if the file is complete or not.
 * -originalFileName (string) : the original file name. -fileCompletion (string) :
 * the file completion formatted with its unit. -originalFileSize (string) : the
 * original file size formatted with its unit.
 * 
 */
function initializeFileUploader(callback) {

	// get the configuration
	$
			.get(
					globalServletMapping + "?action=getConfig",
					function(data) {
						if (data) {
 ...
 */
function clearFileUpload(callback) {
 ...
 */
function cancelFileUpload(fileId, callback) {
	$.get(globalServletMapping + "?action=clearFile&fileId=" + fileId,
			function(e) {
...
function extractFileName(file) {
	var startIndex = (file.indexOf('\\') >= 0 ? file.lastIndexOf('\\') : file
			.lastIndexOf('/'));
	var currentFileName = file.substring(startIndex);
	if (currentFileName.indexOf('\\') === 0
			|| currentFileName.indexOf('/') === 0) {
...
/*
 * referenceToFileElement: reference to the file element progressCallback: a
 * function that will be called to monitor the progress. params: 1 is the
 * fileId, 2 is the percentage as parameter, 3 is the origin element
 * finishCallback: a function that will be called when the process is fully
 * complete. params: 1 is the fileId, 2 is the origin element exceptionCallback:
 * a callback triggered when an exception occurred. params: 1 exception
 * description. fileId: an optional parameter to resume a pending file upload
 */
function fileUploadProcess(referenceToFileElement, progressCallback,
	...
	if (fileId && pendingFiles[fileId]) {
		...
		if (pendingFile.originalFileName != fileName
				|| pendingFile.originalFileSizeInBytes != size) {
			alert("You want to upload a file named '"
					+ fileName
					+ "'("
					+ getFormattedSize(size)
					+ ") but there"
					+ " is already a pending upload for a file named '"
					+ pendingFile.originalFileName
					+ "'("
					+ getFormattedSize(pendingFile.fileCompletionInBytes)
					+ "/"
	...
	// if not, file id is undefined:
		...
		$.get(globalServletMapping + "?action=prepareUpload&fileName="
				+ fileName + "&size=" + size, function(response) {
	    ...
	    if (e.target.readyState == FileReader.DONE) { // DONE == 2
			...
			xhr.onreadystatechange = function() {
				if (xhr.readyState == 4) {
					...
					if (xhr.status != 200) {
						exceptionCallback("Error while uploading slice of byte "
								+ filePosition + "-" + (filePosition + bytesPerChunk));
						return;
					}
...
function startProgressPoller(fileId, progressCallback, referenceToFileElement) {
	...
	// get the configuration
	$.get(globalServletMapping + "?action=getProgress&fileId=" + fileId,
			function(data) {
